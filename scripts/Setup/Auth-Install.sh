...
if [ "$SETUP_TYPE" = "Normal" ]; then
    cmake /home/$SETUP_AUTH_USER/azerothcore/ -DCMAKE_INSTALL_PREFIX=/home/$SETUP_AUTH_USER/server -DCMAKE_C_COMPILER=/usr/bin/clang -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DWITH_WARNINGS=0 -DWITH_COREDEBUG=0 -DTOOLS_BUILD=db-only -DSCRIPTS=dynamic -DMODULES=dynamic -DAPPS_BUILD=auth-only
elif [ "$SETUP_TYPE" = "GDB" ]; then
    cmake /home/$SETUP_AUTH_USER/azerothcore/ -DCMAKE_INSTALL_PREFIX=/home/$SETUP_AUTH_USER/server -DCMAKE_C_COMPILER=/usr/bin/clang -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DWITH_WARNINGS=0 -DWITH_COREDEBUG=1 -DTOOLS_BUILD=db-only -DSCRIPTS=dynamic -DMODULES=dynamic -DAPPS_BUILD=auth-only
fi
