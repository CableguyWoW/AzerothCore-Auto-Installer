...
echo "##########################################################"
echo ""
sudo find /AzerothCore-Auto-Installer/ -type d -name ".git" -prune -o -type f -exec dos2unix {} \;
sudo chmod -R 777 /AzerothCore-Auto-Installer/
...
if [ ! -f "$MYSQL_APT_CONFIG" ]; then
    ...
    wget https://dev.mysql.com/get/mysql-apt-config_0.8.32-1_all.deb -O "$MYSQL_APT_CONFIG"
...
echo ""
echo "cd /AzerothCore-Auto-Installer/scripts/Setup/ && ./Root-Install.sh all"
echo ""
echo "##########################################################"
echo ""
fi
